...
export function ShinyButton({ children, className = "", hover = true, href, shine = true, ...props }: ShinyButtonProps) {
  ...
  const buttonClassName = `group relative inline-flex min-h-11 items-center justify-center gap-2 overflow-hidden rounded-full border border-[#20221F] bg-[#20221F] px-5 font-medium tracking-[-0.01em] text-white dark:border-[#F2F3EE] dark:bg-[#F2F3EE] dark:font-semibold dark:text-[#10110F] ${hoverClassName} ${className}`;
  const content = <>{shine && <span aria-hidden="true" className="pointer-events-none absolute inset-y-0 -left-1/2 w-1/3 -skew-x-12 bg-linear-to-r from-transparent via-white/70 to-transparent animate-[shiny-button_3s_ease-in-out_infinite] motion-reduce:animate-none dark:via-[#10110F]/15" />}<span className="relative z-10 inline-flex items-center gap-2">{children}</span></>;
